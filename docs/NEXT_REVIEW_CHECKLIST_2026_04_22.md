...
- five-image Qwen raw versus `Qwen + LayerForge graph` comparison has been run and summarized in `runs/qwen_five_image_review/`;
...
1. The repo now contains a measured five-image Qwen raw versus hybrid comparison.
...
> LayerForge-X is now a structured, benchmarked layer-representation system with a hybrid Qwen path, recursive peeling, associated-effect layers, and auditable artifact snapshots. The main remaining weakness is not missing implementation but that the effect-layer extractor is still heuristic and should not be oversold.
